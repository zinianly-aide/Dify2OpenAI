...
function extractToolInfo(messages=[]) { const calls = new Map(); const results=[]; for (const m of messages) { if (m.role==='assistant' && Array.isArray(m.tool_calls)) for (const c of m.tool_calls) calls.set(c.id,c); if (m.role==='tool' || m.role==='function') results.push(m); } return { calls, results }; }
...
function fullHistory(messages) { return messages.map(serializeMessage).filter(Boolean).join('\n\n'); }
function deltaHistory(messages) { const last=[...messages].reverse().find(m=>m.role==='user'); return last ? serializeMessage(last) : serializeMessage(messages.at(-1)); }
...
function isInvalidConversation(status, body) { return [400,404].includes(status) && /conversation|not found|invalid/i.test(body||''); }
...
async function handleRequest(req,res,config){
  const data=req.body||{}; const messages=Array.isArray(data.messages)?data.messages:[]; const traceId=randomUUID(); const dshConversationId=dshIdOf(req); const providerId=String(req.headers['x-provider-id']||PROVIDER); const difyAppId=appIdOf(req,config);
  ...
  let remote=conversationStore.get(dshConversationId,providerId,difyAppId); let resolved=resolveConversationState({remoteState:remote,messages,reset:false});
  ...
  if(!result.ok && remote?.conversationId && isInvalidConversation(result.status,result.raw)){
    conversationStore.invalidate(dshConversationId,providerId,difyAppId); resolved=resolveConversationState({remoteState:conversationStore.get(dshConversationId,providerId,difyAppId),messages,remoteInvalid:true});
  ...
  const difyConversationId=result.json?.conversation_id||remote?.conversationId||'';
  if(difyConversationId) remote=conversationStore.set(dshConversationId,providerId,difyAppId,{conversationId:difyConversationId,valid:true,updatedAt:Date.now(),toolSchemaHash:schema.toolSchemaHash});
  let answer=String(result.json?.answer||''); let toolCalls=parseToolCalls(answer); const emitted=[]; const replay=[];
  for(const c of toolCalls){ const args=c.function?.arguments||'{}'; const entry=toolExecutionLedger.begin({providerId,conversationId:dshConversationId,toolCallId:c.id,arguments:args}); trace({traceId,dshConversationId,providerId,difyConversationId,conversationState:resolved.state,toolSchemaHash:schema.toolSchemaHash,toolCallId:c.id,argumentsHash:entry.argumentsHash,toolExecutionStatus:entry.status,contextStrategy:resolved.contextStrategy}); if(entry.replay) replay.push({call:c,result:entry.result}); else emitted.push(c); }
  if(replay.length && !emitted.length){ const replayQuery=replay.map(x=>`tool_result tool_call_id=${x.call.id}: ${x.result}`).join('\n'); const continued=await callDify(config,{...makeBody(difyConversationId,'TOOL_CONTINUE'),query:replayQuery}); if(continued.ok){answer=String(continued.json?.answer||'');toolCalls=parseToolCalls(answer);emitted.push(...toolCalls.filter(c=>!toolExecutionLedger.begin({providerId,conversationId:dshConversationId,toolCallId:c.id,arguments:c.function?.arguments||'{}'}).replay));} }
