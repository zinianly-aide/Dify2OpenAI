...
export class GatewayObserver {
  ...
  observe(req, res, routing = {}) {
    ...
    if (req.body && Array.isArray(req.body.messages) && compression.result.mode !== 'none') {
      req.body.messages = compression.messages;
    }
    res.locals ??= {};
    res.locals.gatewayCompressionResult = compression.result;
    ...
    return { canonicalRequest, profile, decision, compressionResult: compression.result };
  }
}
