...
export class MemoryConversationStore {
  ...
  _root(dshConversationId) { if (!this.conversations.has(dshConversationId)) this.conversations.set(dshConversationId, { providers: new Map() }); return this.conversations.get(dshConversationId); }
  ...
  set(dshConversationId, providerId, difyAppId, state) { const root = this._root(dshConversationId); const key = this._providerKey(providerId, difyAppId); root.providers.set(key, { ...(root.providers.get(key) || {}), ...state }); return root.providers.get(key); }
  invalidate(dshConversationId, providerId, difyAppId = '') { const state = this.get(dshConversationId, providerId, difyAppId); if (state) state.valid = false; return state; }
