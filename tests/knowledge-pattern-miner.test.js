...
test('checkpoint success strengthens pattern while failed intervention lowers confidence', () => {
  ...
  const supported = find(miner.mine([
    exp(1, { checkpointCreated: true }), exp(2, { checkpointCreated: true }), exp(3, { checkpointCreated: true }),
  ]), 'checkpoint-reduces-backend-context');
  const weakened = find(miner.mine([
    exp(1, { checkpointCreated: true }), exp(2, { checkpointCreated: true }), exp(3, { checkpointCreated: true, success: false }),
  ]), 'checkpoint-reduces-backend-context');
...
test('cross-client evidence increases transferability without storing session identity', () => {
  const miner = new PatternMiner();
  const single = find(miner.mine([
    exp(1, { clientType: 'dsh', backendId: 'b1', model: 'm1' }),
    exp(2, { clientType: 'dsh', backendId: 'b1', model: 'm1' }),
    exp(3, { clientType: 'dsh', backendId: 'b1', model: 'm1' }),
  ]), 'stateful-context-amplification');
  const cross = find(miner.mine([
    exp(1, { clientType: 'dsh', backendId: 'b1', model: 'm1' }),
    exp(2, { clientType: 'codex', backendId: 'b2', model: 'm2' }),
    exp(3, { clientType: 'openai-compatible', backendId: 'b3', model: 'm3' }),
  ]), 'stateful-context-amplification');
...
test('single version-only issue stays VERSION_SPECIFIC', () => {
  const miner = new PatternMiner();
  const pattern = find(miner.mine([exp(1, { checkpointCreated: true, versionSpecific: true })]), 'checkpoint-reduces-backend-context');
...
test('repeated evidence does not duplicate patterns in PatternStore', () => {
  ...
  assert.equal(store.getPattern(pattern.patternId).patternId, pattern.patternId);
});
...
test('contradictory checkpoint evidence becomes CONTRADICTED', () => {
  ...
  const pattern = find(miner.mine([
    exp(1, { checkpointCreated: true, success: true }),
    exp(2, { checkpointCreated: true, success: false }),
    exp(3, { checkpointCreated: true, success: false }),
  ]), 'checkpoint-reduces-backend-context');
...
test('promotion threshold emits SKILL_CANDIDATE signal only', () => {
  ...
  const experiences = Array.from({ length: 9 }, (_, i) => exp(i + 1, {
