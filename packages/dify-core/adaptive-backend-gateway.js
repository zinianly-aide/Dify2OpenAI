    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      if (migrationRequired) {
        ...
        if (!checkpoint && this.checkpointManager && Array.isArray(input.messages) && input.messages.length) {
          ...
          if (sourceRemote) {
            const created = this.checkpointManager.create({
              sessionId, backendId: sourceBackendId, providerId, appId,
              sourceGeneration: sourceRemote.generation, contextVersion: (sourceRemote.contextVersion || sourceRemote.generation || 1) + 1,
              messages: input.messages, compressedMessages: input.messages, system: input.system, tools: input.tools || [],
        ...
        migration = this.migrationPlanner.plan({
          sessionId, sourceBackendId, targetBackendId: backendId, providerId, appId,
          targetCapabilities: backend.capabilities, checkpoint, canonicalContextAvailable: Array.isArray(input.messages) && input.messages.length > 0,
        });
      ...
      if (backend.capabilities.contextMode === BackendContextMode.STATEFUL) {
        if (!migrationRequired && targetRemote?.conversationId) conversationId = targetRemote.conversationId;
      ...
      try {
        ...
        if (backend.capabilities.contextMode === BackendContextMode.STATEFUL) {
          ...
          if (targetRemote?.conversationId) {
            if (targetRemote.conversationId !== result.conversationId) throw new Error('ACTIVE_GENERATION_CONVERSATION_ID_IMMUTABLE');
          } else {
            this.conversationStore.set(sessionId, providerId, appId, { backendId, conversationId: result.conversationId, checkpointId: checkpoint?.checkpointId, contextVersion: checkpoint?.contextVersion });
          }
        }
        ...
        if (!isFallbackEligible(error) || index === candidates.length - 1) {
          error.routing = { selectedBackend: backendId, previousBackend: currentBackendId, migrationRequired, reasonCodes: routing.reasonCodes, fallbackChain: routing.fallbackChain, fallbackUsed };
          error.migration = { started: migrationRequired, success: false, failureReason: String(error?.code || error?.name || 'BACKEND_ERROR'), sourceBackendId, targetBackendId: backendId };
          throw error;
        }
      }
    }
