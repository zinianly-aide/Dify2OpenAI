...
export class ToolSchemaRegistry {
  ...
  key(dshConversationId, providerId, difyAppId = '', backendId = '', generation = '') {
    return `${dshConversationId}::${providerId}::${difyAppId}::${backendId}::${generation}`;
  }
  resolve({ dshConversationId, providerId, difyAppId = '', backendId = '', generation = '', tools = [] }) {
    ...
    const key = this.key(dshConversationId, providerId, difyAppId, backendId, generation);
