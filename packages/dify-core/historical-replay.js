...
function compressionFactor(mode) {
  if (mode === 'heavy') return 0.65;
  if (mode === 'light') return 0.82;
  if (mode === 'tool_prune') return 0.97;
  return 1;
}
...
function cloneRegistry(registry, policy) {
  ...
  const entries = registry.list({ enabledOnly: false }).map((backend) => ({
    ...
    statefulContext: backend.capabilities.contextMode === 'stateful',
...
function syntheticToolInput(event, threshold) {
  ...
  const observedScore = Number.isFinite(Number(event.toolPruningConfidenceScore))
    ? Number(event.toolPruningConfidenceScore)
    : event.toolPruningConfidence === 'high' ? 0.75 : 0;
  ...
  const explicitRequiredTools = observedScore >= 1 && count ? ['replay_tool_0'] : [];
  const policy = new ToolRelevancePolicy({ pruningConfidenceThreshold: threshold });
  return policy.classify({
    canonicalRequest: { clientType: event.clientType, taskType: event.taskType },
    tools,
    profile,
...
function toolPrediction(event, policyResult) {
  ...
  const observedRatio = beforeCount > 0 && Number(event.toolCountAfter) < beforeCount
    ? Math.max(1 / beforeCount, Number(event.toolCountAfter) / beforeCount)
    : 0.75;
...
function replayOne(event, policy, registry) {
  const compressionPolicy = new CompressionPolicy(policy.compression);
  const compression = compressionPolicy.decide({
    contextUtilization: event.contextUtilization,
    clientType: event.clientType,
    backendId: event.backendId,
    model: event.model,
    estimatedPromptTokens: event.estimatedInputTokens,
    contextWindow: event.contextWindow,
    messageCount: 0,
    toolSchemaTokens: event.toolSchemaTokensBefore,
  });
  let estimatedTokens = estimateTokens(event, compression);
  const checkpointPolicy = new CheckpointRecommendation({ config: policy.checkpoint });
  const checkpoint = checkpointPolicy.recommend({
    compressionResult: { unableToReachTarget: false },
    reconciliation: {
      contextAmplification: Number.isFinite(Number(event.contextAmplification)) ? Number(event.contextAmplification) : undefined,
      backendContextUtilization: Number.isFinite(Number(event.contextUtilization)) ? Number(event.contextUtilization) : undefined,
    },
  });
  if (checkpoint.recommended) estimatedTokens = Math.round(estimatedTokens * 0.65);

  const toolPolicyResult = syntheticToolInput(event, policy.tool.pruningConfidenceThreshold);
  const tool = toolPrediction(event, toolPolicyResult);
  ...
  if (replayRegistry) {
    const router = new DeterministicBackendRouter({ registry: replayRegistry, policyVersion: 'offline-replay-router-v1' });
    routing = router.decide({
      taskType: event.taskType,
      currentBackendId: event.previousBackendId || undefined,
      estimatedTokens,
      contextUtilization: event.contextUtilization,
      requiresTools: event.requiresTools,
      hasImages: event.hasImages,
      reasoningRequired: event.reasoningRequired,
      streamingRequired: event.streamingRequired,
    });
  }
  ...
  return {
    estimatedTokens,
    estimatedCost: costEstimate(event, estimatedTokens, tool.schemaTokens),
    compression: compression.mode !== 'none',
    checkpoint: checkpoint.recommended,
    fallback: fallbackPredicted(event, routing),
    toolPruning: tool.pruning,
    overflow,
...
function aggregate(results) {
  return Object.freeze({
    ...
    estimatedCost: results.reduce((sum, item) => sum + item.estimatedCost, 0),
    compressionCount: results.filter((item) => item.compression).length,
    checkpointCount: results.filter((item) => item.checkpoint).length,
    fallbackCount: results.filter((item) => item.fallback).length,
    toolPruningCount: results.filter((item) => item.toolPruning).length,
    predictedOverflowCount: results.reduce((sum, item) => sum + item.overflow, 0),
    predictedToolSchemaTokens: results.reduce((sum, item) => sum + item.schemaTokens, 0),
  });
}
...
export class HistoricalReplay {
  constructor({ registry, replayVersion = HISTORICAL_REPLAY_VERSION, analyzerVersion = 'offline-policy-analyzer-v1' } = {}) {
  ...
  replay({ snapshot, baselinePolicy = {}, candidate } = {}) {
    ...
    if (!validation.valid) {
      const error = new Error(`POLICY_CANDIDATE_INVALID:${validation.errors.join(',')}`);
      error.code = 'POLICY_CANDIDATE_INVALID';
    ...
    const unsupportedDecisionCount = candidateResults.reduce((sum, item) => sum + item.unsupported, 0);
    ...
    return Object.freeze({
      ...
      dataset: Object.freeze({
        ...
        basePolicyMismatchCount,
      }),
      semantics: Object.freeze({
        historical: 'observed',
        baseline: 'deterministic_replay_estimated',
        candidate: 'deterministic_replay_predicted',
        futureLatency: 'not_replayed',
        futureAnswerQuality: 'not_replayed',
        futureToolSuccess: 'not_replayed',
      }),
      baseline: baselineMetrics,
      candidate: candidateMetrics,
      delta: Object.freeze({
        tokenPct: pct(candidateMetrics.estimatedTokens, baselineMetrics.estimatedTokens),
        costPct: pct(candidateMetrics.estimatedCost, baselineMetrics.estimatedCost),
        checkpointPct: pct(candidateMetrics.checkpointCount, baselineMetrics.checkpointCount),
        fallbackPct: pct(candidateMetrics.fallbackCount, baselineMetrics.fallbackCount),
        overflowPct: pct(candidateMetrics.predictedOverflowCount, baselineMetrics.predictedOverflowCount),
      }),
      risk: Object.freeze({
        routingDrift: events.length ? driftCount / events.length : 0,
        toolRecoveryRisk,
