...
export const DEFAULT_POLICY_LIMITS = Object.freeze({
  compression: Object.freeze({
    toolPruneThreshold: [0.05, 0.90], lightThreshold: [0.10, 0.95], heavyThreshold: [0.20, 0.98], forceThreshold: [0.30, 1.00], targetUtilization: [0.40, 0.90],
  }),
  checkpoint: Object.freeze({ backendContextUtilizationThreshold: [0.50, 0.99], amplificationThreshold: [1.00, 10.00] }),
  backendHealth: Object.freeze({ minimumSamples: [1, 1000], unavailableConsecutiveFailures: [1, 20], degradedFailureRate: [0.01, 0.90], unavailableFailureRate: [0.05, 1.00] }),
  tool: Object.freeze({ pruningConfidenceThreshold: [0.10, 1.00], recoveryLimit: [0, 3] }),
  backendPriority: Object.freeze({ value: [0, 10000] }),
});

const BASE_COMPRESSION_THRESHOLDS = Object.freeze({ toolPruneThreshold: 0.55, lightThreshold: 0.70, heavyThreshold: 0.82, forceThreshold: 0.92 });

function inRange(value, range) { const n = Number(value); return Number.isFinite(n) && n >= range[0] && n <= range[1]; }
...
export function validatePolicyChanges(changes, limits = DEFAULT_POLICY_LIMITS) {
  ...
  for (const section of Object.keys(changes).sort()) {
    if (!allowedSections.has(section)) { errors.push(`POLICY_FIELD_NOT_ALLOWED:${section}`); continue; }
    if (!plainObject(changes[section])) { errors.push(`POLICY_SECTION_INVALID:${section}`); continue; }
  ...
  if (plainObject(changes.compression) && Object.keys(changes.compression).some((field) => field.endsWith('Threshold'))) {
    const merged = { ...BASE_COMPRESSION_THRESHOLDS, ...changes.compression };
    const { toolPruneThreshold: toolPrune, lightThreshold: light, heavyThreshold: heavy, forceThreshold: force } = merged;
...
function createCandidate({ basePolicyVersion, changes, evidence, hypothesis, expectedImpact, confidence, createdAt }) {
  ...
  return Object.freeze({ candidateId: `pc-${sha256(canonicalJson(identity)).slice(0, 20)}`, basePolicyVersion, changes: Object.freeze(changes), evidence: Object.freeze(evidence), hypothesis, expectedImpact: Object.freeze(expectedImpact), confidence, createdAt, schemaVersion: POLICY_CANDIDATE_SCHEMA_VERSION });
}

export const DEFAULT_CANDIDATE_GENERATOR_CONFIG = Object.freeze({ minimumEvidence: 20, highContextRate: 0.50, highBackendP95Ms: 2500, highToolSchemaTokensPerRequest: 1200, lowToolRecoveryRate: 0.02, highToolRecoveryRate: 0.06 });
...
export class DeterministicPolicyCandidateGenerator {
  ...
  generate({ analysis, snapshot, basePolicyVersion, baselinePolicy = {}, compatibleBackendIds = [] } = {}) {
    ...
    if (Number(overall.highContextRate || 0) >= this.config.highContextRate) {
      ...
      if (next < current) candidates.push(createCandidate({ basePolicyVersion, changes: { checkpoint: { backendContextUtilizationThreshold: next } }, evidence: { requestCount: count, highContextRate: overall.highContextRate, checkpointFrequency: overall.checkpointFrequency }, hypothesis: 'Historical context utilization is persistently high; checkpointing earlier may reduce predicted overflow pressure.', expectedImpact: { overflow: 'decrease_predicted', checkpoint: 'increase_predicted' }, confidence: count >= this.config.minimumEvidence * 3 ? 'high' : 'medium', createdAt }));
    }
    for (const group of analysis.groups || []) {
      ...
      if (backendId && group.metrics?.requestCount >= this.config.minimumEvidence && Number.isFinite(p95) && p95 >= this.config.highBackendP95Ms && compatibleBackendIds.some((id) => id !== backendId)) {
        ...
        candidates.push(createCandidate({ basePolicyVersion, changes: { backendPriority: { [backendId]: Math.min(10000, current + 20) } }, evidence: { backendId, requestCount: group.metrics.requestCount, observedLatencyP95Ms: p95, compatibleBackendCount: compatibleBackendIds.length }, hypothesis: 'Observed p95 latency is high and compatible alternatives exist; lowering this backend routing preference may reduce latency exposure.', expectedImpact: { routing: 'shift_predicted', latency: 'not_replayed_real_latency' }, confidence: 'medium', createdAt }));
      }
    }
    ...
    if (avgSchema >= this.config.highToolSchemaTokensPerRequest && recoveryRate <= this.config.lowToolRecoveryRate) {
      candidates.push(createCandidate({ basePolicyVersion, changes: { tool: { pruningConfidenceThreshold: Math.max(0.10, currentPrune - 0.05) } }, evidence: { requestCount: count, avgToolSchemaTokensBefore: avgSchema, observedToolRecoveryRate: recoveryRate }, hypothesis: 'Tool schema cost is high while observed recovery is low; slightly stronger deterministic pruning may reduce schema tokens.', expectedImpact: { toolSchemaTokens: 'decrease_predicted', toolRecoveryRisk: 'increase_proxy_possible' }, confidence: 'medium', createdAt }));
    } else if (recoveryRate >= this.config.highToolRecoveryRate) {
      candidates.push(createCandidate({ basePolicyVersion, changes: { tool: { pruningConfidenceThreshold: Math.min(1, currentPrune + 0.08) } }, evidence: { requestCount: count, observedToolRecoveryRate: recoveryRate }, hypothesis: 'Observed full-tool recovery is elevated; more conservative pruning may reduce recovery risk.', expectedImpact: { toolSchemaTokens: 'increase_predicted', toolRecoveryRisk: 'decrease_predicted' }, confidence: 'medium', createdAt }));
    }
    ...
    return Object.freeze({ status: unique.length ? 'CANDIDATES' : 'NO_CANDIDATE', reasonCodes: Object.freeze(unique.length ? ['DETERMINISTIC_EVIDENCE_RULE_MATCH'] : ['NO_RULE_WITH_SUFFICIENT_EVIDENCE']), candidates: Object.freeze(unique) });
  }
}
