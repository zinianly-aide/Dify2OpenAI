...
function unique(experiences, selector) {
  return [...new Set(experiences.map(selector).filter((v) => v && v !== 'unknown' && !String(v).endsWith('-unknown')))].sort();
}
function diversity(experiences, selector) { return unique(experiences, selector).length; }
  ...
  mine(experiences = []) {
    ...
    for (const rule of RULES) {
      ...
      patterns.push(createKnowledgePattern({
        ...
        conditions: { semanticRule: rule.key, environment: { clientTypes, backendTypes, backendIdHashes, modelFamilies, policyVersions } },
        ...
        evidence: { observationCount: matched.length, successCount, failureCount, firstSeen: matched.map((e) => e.timestamp).sort()[0] || null, lastSeen: matched.map((e) => e.timestamp).sort().at(-1) || null, clientDiversity, backendDiversity, modelDiversity },
        confidence,
        impact,
        transferability,
